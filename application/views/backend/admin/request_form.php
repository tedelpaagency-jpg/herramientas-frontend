...
<div class="modal-body">
    <form action="<?= base_url(); ?>portal/requests/updateRequestInfo/<?= base64_encode($param2); ?>" method="POST" />
        ...
        <?php if($filas->status == 1): ?>
         ...
         <?php endif; ?>
         <?php if($filas->status == 2): ?>
          <div class="mb-3">
            <label for="email" class="form-label">Comisión:</label>
            <input type="text" class="form-control"  value="<?php echo $filas->commition; ?>" readonly>         
        </div>
         <?php endif; ?>
              <?php if($filas->status == 0): ?>
          <div class="mb-3">
            <label for="email" class="form-label">Motivo:</label>
            <input type="text" class="form-control"  value="<?php echo $filas->motivo; ?>" readonly>         
        </div>
         <?php endif; ?>
    </form>
</div>
