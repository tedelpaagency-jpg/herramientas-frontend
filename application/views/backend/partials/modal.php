...
<div id="cwRecipeModal" class="cw-modal-overlay">
    <div class="cw-modal-content">
        <div id="cwRecipeContent"></div>
    </div>
</div>

<div id="cwHistoryModal" class="cw-modal-overlay">
    <div class="cw-modal-content">
        <div id="cwHistoryContent"></div>
    </div>
</div>
...
{
    $.ajax({
        url: base_url + "portal/prescriptions/ajax_recipe/" + id,
        type: "GET",
        beforeSend:function(){
            $("#cwRecipeContent").html('<div class="text-center p-5"><i class="fa fa-spinner fa-spin fa-2x"></i></div>');
        },
        success:function(html){

            $("#cwRecipeContent").html(html);

            lucide.createIcons();

            openModal("cwRecipeModal");
        }
    });
}
...
{
    $.ajax({
        url: base_url + "portal/prescriptions/ajax_history/" + patient_id,
        type: "GET",
        beforeSend:function(){
            $("#cwHistoryContent").html('<div class="text-center p-5"><i class="fa fa-spinner fa-spin fa-2x"></i></div>');
        },
        success:function(html){

            $("#cwHistoryContent").html(html);

            lucide.createIcons();

            openModal("cwHistoryModal");
        }
    });
}
...
function openModal(modalId) {
        ...
        // Timeout ligero para que la transición de CSS se aplique sobre el display: flex
        setTimeout(() => {
