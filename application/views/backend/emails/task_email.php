...
<head>
    ...
    <title>Notificación Premium de Tarea</title>
    <style>
...
<body>
    <?php
        ...
        $task = $query->row();
        $agency_id = $this->crud_model->getAgency('user',$task->user_id); 
    ?>
    <div class="wrapper">
        ...
        <table class="main-container" align="center" cellpadding="0" cellspacing="0">
            ...
            <tr>
                <td style="padding: 0 24px 32px 24px;">
                    <h2 style="margin: 0 0 12px 0; font-size: 22px; font-weight: 800; color: #1e293b; letter-spacing: -0.025em;">Recordatorio de Tarea</h2>
                    ...
                    <table width="100%" style="background-color: #f8fafc; border-radius: 12px; border: 1px solid #e2e8f0; overflow: hidden;">
                        <tr>
                            <td style="padding: 20px; border-left: 4px solid #1877f2;">
                                <div style="font-size: 12px; text-transform: uppercase; color: #64748b; font-weight: 700; letter-spacing: 0.05em; margin-bottom: 6px;">Identificador de Tarea</div>
                                <div style="font-size: 18px; color: #0f172a; font-weight: 700; margin-bottom: 16px;"><?= $task->name ?></div>
                                ...
                                <table width="100%">
                                    <tr>
                                        <td style="font-size: 14px; color: #475569;">
                                            <span style="display: inline-block; background-color: #fee2e2; color: #991b1b; padding: 4px 8px; border-radius: 4px; font-size: 11px; font-weight: 800; vertical-align: middle; margin-right: 8px;">URGENTE</span>
                                            <?php
                                                $now = new DateTime();
                                                
                                                $taskDateTime = new DateTime($task->date . ' ' . $task->time);
                                                
                                                $diff = $now->diff($taskDateTime);
                                                
                                                $hours = ($diff->days * 24) + $diff->h;
                                                
                                                echo 'Vence en: <strong>' . $hours . ' horas</strong>';
                                            ?>
                                        </td>
                                    </tr>
                                </table>
                    ...
                    <table width="100%">
                        <tr>
                            <td width="52" valign="top">
                                <img src="<?= $this->crud_model->getPhoto('user',$task->user_id); ?>" alt="Responsable" class="profile-img">
                            </td>
                                ...
                                <td style="padding-left: 16px; vertical-align: middle;">
                                    ...
                                    <div style="font-size: 15px; font-weight: 600; color: #1e293b;"><?= $this->crud_model->getName('user',$task->user_id); ?></div>
                                    <div style="font-size: 14px; color: #1877f2; font-weight: 700; margin-top: 2px;">
                                        ...
                                        +<?= $this->crud_model->getPhone('user',$task->user_id); ?>
                                    </div>
                                </td>
                            </tr>
                        </table>
