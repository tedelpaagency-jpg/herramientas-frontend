...
<div class="modal-header bg-current">
    <div class="border-0 d-flex">
        ...
        <h4 class="font-xs text-white fw-600 ms-4 mb-0 mt-2" id="exampleModalLabel"><?php echo isset($user) ? 'Actualizar tienda' : 'Agregar tienda'; ?></h4>    
    </div>
...
<div class="modal-body row">
        ...
        <div class="col-md-12">
            <div class="form-group">
                <label for="ruc" class="mont-font fw-600 font-xsss">DNI</label>
        ...
        <div class="col-md-12">
            <div class="form-group mb-3">
                ...
                <br>
                <select id="provincia" disabled class="form-control mb-3" name="provincia" required="">
                    <option value="" class="form-control">Selecciona una provincia</option>
                </select>
                <br>
                <select id="canton" disabled class="form-control mb-3" name="canton" required="">
...
<div class="modal-footer">
    ...
    <button type="submit" class="btn btn-primary text-center  text-white" id="btnsubmit"><?php echo isset($user) ? 'Actualizar tienda' : 'Agregar tienda'; ?></button>
</div>
</form>
<script>
...
$('#userForm').on('submit', function(e) {
    ...
    $("#pais").on("change", function () {
        ...
        if (pais_id) {
            $.getJSON("<?= base_url('portal/obtener_provincias/'); ?>" + pais_id, function (data) {
                $.each(data, function (index, provincia) {
                    $("#provincia").append($("<option>", { value: provincia.id, text: provincia.nombre }));
                });
    ...
    $("#provincia").on("change", function () {
        ...
        if (provincia_id) {
            $.getJSON("<?= base_url('portal/obtener_cantones/'); ?>" + provincia_id, function (data) {
                $.each(data, function (index, canton) {
                    $("#canton").append($("<option>", { value: canton.id, text: canton.nombre }));
                });
