...
import { 
  ...
  TrendingUp, Users, DollarSign, Award, X, Sparkles, ExternalLink, RefreshCw, ChevronLeft
} from 'lucide-react';
...
export const HunterStoresPage: React.FC = () => {
  ...
  const handleSaveStore = async (e: React.FormEvent) => {
    ...
    try {
      if (editingStore) {
        ...
        toast.success('Nueva Tienda Hunter registrada correctamente');
      }
  ...
  const handleConfirmApprove = async () => {
    ...
    if (isNaN(comm) || comm < 0) {
      toast.error('Ingrese un porcentaje de comisión válido');
      return;
    }

    try {
      await hunterService.approveRequest(approvingRequest.id, comm);
      toast.success(`Solicitud #${approvingRequest.id} aprobada con ${comm}% de comisión`);
  ...
  return (
    <div className="space-y-6">
                        ...
                        profileData.requests.map((req) => (
                          <tr key={req.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                            ...
                            <td className="p-3 font-black text-slate-900 dark:text-white">
                              {req.status === 2 ? `${req.commission_percentage}%` : '—'}
                            </td>
                            <td className="p-3">
                              {req.status === 2 ? (
      ...
      {isCreateModalOpen && (
        <Portal>
          <div className="fixed inset-0 top-0 left-0 w-screen h-screen z-[9999] bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 max-w-lg w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto custom-scrollbar">
      ...
      {qrModalStore && (
        <Portal>
          <div className="fixed inset-0 top-0 left-0 w-screen h-screen z-[9999] bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 max-w-sm w-full p-6 text-center shadow-2xl space-y-4">
