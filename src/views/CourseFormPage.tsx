...
export const CourseFormPage: React.FC = () => {
  ...
  const startAddSectionToModule = (moduleId?: number) => {
    resetSectionForm();
    const targetId = moduleId || (modules.length > 0 ? modules[0].id : null);
    if (!targetId && modules.length === 0) {
      toast.error('Primero debes crear un módulo para poder agregar secciones.');
      return;
    }
    setTargetModuleId(targetId);
  ...
  const secContentRef = useRef<HTMLTextAreaElement | null>(null);

  const insertFormatTag = (openTag: string, closeTag: string = '') => {
    const textarea = secContentRef.current;
    if (!textarea) {
      if (!closeTag) {
                                ...
                                <div className="space-y-2 pt-1">
                                  ...
                                  {!showSecPreview && (
                                    <div className="flex flex-wrap items-center gap-1.5 p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
                                      ...
                                      <div className="h-4 w-px bg-slate-200 dark:bg-slate-800 mx-1" />
                                      <button type="button" onClick={() => insertFormatTag('<br />')} className="px-2 py-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-mono text-[11px] font-bold" title="Salto de línea">&lt;br&gt;</button>
                                      <button type="button" onClick={() => insertFormatTag('<p>', '</p>')} className="px-2 py-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-mono text-[11px] font-bold" title="Párrafo">&lt;p&gt;</button>
                                      <div className="h-4 w-px bg-slate-200 dark:bg-slate-800 mx-1" />
                                  ...
                                  {showSecPreview ? (
                                    <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 min-h-[140px] text-sm text-slate-800 dark:text-slate-200 prose dark:prose-invert max-w-none whitespace-pre-wrap">
                                      {secContent.trim() ? (
                                    ...
                                    <textarea
                                      ref={(el) => { secContentRef.current = el; }}
                                      rows={5}
                                      value={secContent}
